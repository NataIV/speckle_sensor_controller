...
// INSTANCIACION DEL CONFIGURADOR DE AMPLIFICADORES, TODOS MISMA GANANCIA
wire to_chip_amp_clk;
wire to_chip_amp_rst;
wire to_chip_amp_ena;
wire to_chip_amp_data;
wire to_amp_config_go = i_optreg[2];
wire from_amp_config_done;
...
amp_config #(
    ...
    .clk ( clk ),
    .rst ( rst ),
    .i_go ( to_amp_config_go ),
    .i_amp_value ( i_amp_value_reg ),
    .i_clk_div  ( i_clk_div_sr ), //Misma configuracion de clock que el registro de filas y columnas
    .o_amp_clk  ( chip_amp_clk ),
    .o_amp_data ( chip_amp_data ),
    .o_amp_en   ( chip_amp_ena ),
    .o_amp_nrst ( chip_amp_rst ),
    .o_done     ( from_amp_config_done )
);
...
assign o_status[3:0] = {
    ...
    to_cfg_fsm_start,
    (from_top_fsm_done & from_amp_config_done)
};
